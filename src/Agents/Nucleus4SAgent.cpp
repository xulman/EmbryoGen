...
void Nucleus4SAgent::getCurrentOffVectorsForCentres(Vector3d<G_FLOAT> offs[4]) {
	...
	// calculate how much are the 2nd and 3rd spheres off their expected
	// positions
	offs[1] = refCentre;
	...
	offs[0] = refCentre;
	offs[0] -=
	    (centreDistance[0] / (0.5f * centreDistance[1]) + 1.0f) * refAxis;
	offs[0] -= futureGeometry.centres[0];

	// calculate how much is the 4th sphere off its expected position
	offs[3] = refCentre;
	offs[3] +=
	    (centreDistance[2] / (0.5f * centreDistance[1]) + 1.0f) * refAxis;
...
void Nucleus4SAgent::drawForDebug(DisplayUnit& du) {
	...
	if (detailedDrawingMode) {
		...
		dID += 32767; // we will be using IDs from the upper end of the reserved
		              // interval
		...
		du.DrawLine(dID++, futureGeometry.centres[1],
		            futureGeometry.centres[1] + sOff[1], 3);
		du.DrawLine(dID++, futureGeometry.centres[2],
		            futureGeometry.centres[2] + sOff[2], 3);
		du.DrawLine(dID++, futureGeometry.centres[3],
		            futureGeometry.centres[3] + sOff[3], 3);
	}
}
