...
void ShapeHinter::drawForDebug(i3d::Image3d<i3d::GRAY16>& img) {
	...
	const Vector3d<G_FLOAT> off(img.GetOffset().x, img.GetOffset().y,
	                            img.GetOffset().z);
	...
	for (curPos.z = minSweepPX.z; curPos.z < maxSweepPX.z; curPos.z++)
		for (curPos.y = minSweepPX.y; curPos.y < maxSweepPX.y; curPos.y++)
			for (curPos.x = minSweepPX.x; curPos.x < maxSweepPX.x; curPos.x++) {
				...
				// NB: AABB.exportInPixelCoords() assures that voxel centres
				// fall into AABB
				...
				if (centrePX.x >= distImg.GetSizeX() ||
				    centrePX.y >= distImg.GetSizeY() ||
				    centrePX.z >= distImg.GetSizeZ())
					report::debugMessage(fmt::format(
					    "{} gives counter-voxel {} (to voxel {}) outside of "
					    "the distImg",
					    getSignature(), toString(centrePX), toString(curPos)));

				// extract the value from the distImg
				const float dist =
				    distImg.GetVoxel(centrePX.x, centrePX.y, centrePX.z);

				if (dist < 0 ||
				    (dist == 0 &&
				     model == ScalarImg::DistanceModel::ZeroIN_GradOUT)) {
					i3d::GRAY16 val =
					    img.GetVoxel(curPos.x, curPos.y, curPos.z);
					if (val > 0 && val != (i3d::GRAY16)ID)
						report::debugMessage(
						    fmt::format("{} overwrites mask of {} at {}",
						                getSignature(), val, toString(curPos)));
