...
class Texture {
  public:
	...
	/** state of the random number generator that is associated with this
	 * texture instance */
	...
	checkAndIncreaseCapacity(const unsigned int expectedNoOfNewDots = 10000) {
		if (dots.capacity() - dots.size() < expectedNoOfNewDots) {
			...
			report::debugMessage(
			    fmt::format("increased capacity to {} dots", dots.capacity()));
		}

		return dots.capacity();
	}

	/** shortcut method: returns the number of dots currently stored in this
	 * Texture */
	size_t size(void) const { return dots.size(); }

	/** shortcut method: returns the total capacity of how many dots can be
	   currently stored in this Texture before
	   container-capacity-increase-machinery gets triggered */
	size_t capacity(void) const { return dots.capacity(); }
	...
	void setupImageForRasterizingTexture(
	    i3d::Image3d<VT>& img,
	    const Vector3d<float>& imgRes,
	    const Geometry& geom,
	    const Vector3d<short>& pxFrameWidth = Vector3d<short>(2)) const {
		geom.AABB.adaptImage(img, imgRes, pxFrameWidth);
	}

	/** populate (actually add, not reset) the dot list by sampling the
	   rasterized texture (stored in 'img') respecting the actual shape of the
	   agent ('geom'), and considering given 'quantization' */
	template <typename VT>
	void sampleDotsFromImage(const i3d::Image3d<VT>& img,
	                         const Spheres& geom,
	                         const VT quantization = 1);

	/** Populates the given 'geom' union of spheres with a Perlin texture. The
	   texture will be sampled/rendered at the given 'textureResolution' (in
	   pixels/micron), will be intensity-shifted to obtain the given
	   'textureAverageIntensity', and will be created according to the four
	   Perlin texture parameters ('var', 'alpha', 'beta', 'n'). The rendered
	   texture will be sampled (converted into this->dots) using the given
	   'quantization' factor. That means, the number of dots that occupy any
	    texture voxel is given by the voxel's intensity over the 'quantization':
	   the higher the parameter is, the fewer dots are created. If
	   'shouldCollectOutlyingDots' is true, the created dots (incl. any possibly
	   already present dots) are checked against the given 'geom' and shifted if
	   necessary to assure that they are always within the given union of
	   spheres (the 'geom').

	    Note that the Perlin noise created with the default parameters typically
	   creates images with intensities within the range [-0.5;+0.5]. The
	   'textureAverageIntensity' is only shifting the range into
	   [textureAverageIntensity-0.5,textureAverageIntensity+0.5]. Hence, with
	   the higher value of the 'textureAverageIntensity', the contrast (ratio of
	...
	/** renders the current content of the this->dots list into the given
	 * phantom image */
	void renderIntoPhantom(i3d::Image3d<float>& phantoms,
	                       const float quantization = 1);
};

/**
 * Utility class that maintains texture particles, e.g. fluorescence dots, that
 * assumes that one such particle represents a 'quantum' amount of (equally
 * exposed/bleached) dots. The quantum is defined as a number of fluorescence
 * molecules equally spread throughout given (axis aligned) box. Often, one
 * chooses the box size to correspond with the volume of one voxel in the
 * phantom image. Furthermore, the number of the fl. molecules (the quantum
 * size) is a compromise to obtain phantom images with desired intensity levels
 * while maintaining computational tractability (that is, not having exceedingly
 * long list with (small) quanta).
 */
class TextureQuantized : public Texture {
  public:
	/** c'tor that requires explicitly the number of fl. molecules per
	 * volumetric box */
	TextureQuantized(const unsigned int expectedNoOfQuantumDots,
	                 const Vector3d<float>& _boxSize,
	                 const Vector3d<short>& _qCounts)
	    : Texture(expectedNoOfQuantumDots), boxSize(_boxSize),
	      qCounts(_qCounts) {
		checkCountsPerAxis();
	}

	/** a utility c'tor that estimates the quantum box from voxel size and,
	   essentially, a phantom image intensity increase/gain */
	TextureQuantized(const unsigned int expectedNoOfQuantumDots,
	                 const Vector3d<float>& imgRes,
		...
		report::debugMessage(fmt::format(
		    "Converting inputs to: boxSize={} um, qCounts={} sub-dots",
		    toString(boxSize), toString(qCounts)));
		checkCountsPerAxis();
	}

	Vector3d<short> getCountsPerAxis(const Vector3d<float>& imgRes,
	                                 const short quantumGainPerVoxel) const {
		// dots per voxel
		...
		// should hold x*y*z >= original input quantum, ideally closest possible
		// to the equality
	...
	void checkCountsPerAxis() const {
		if (!qCounts.elemIsGreaterThan(Vector3d<short>(0)))
			throw report::rtError(
			    "Quantum cube must have at least one dot along every axis");
	}
	...
	/** counts of quantum dots along every axis, multiplying the .x, .y and .z
	   gives the total contribution of this one quantum box */
	const Vector3d<short> qCounts;

	/** renders (quantum-wise) the current content of the this->dots list into
	   the given phantom image, the contributed intensity to the image should be
	   'quantum'-times greater than what would provide the upstream, non-quantum
	   Texture::renderIntoPhantom(phantoms,1.0) */
	void renderIntoPhantom(i3d::Image3d<float>& phantoms);
};

/**
 * This class can update coordinates of the texture dots on-the-go as the
 * underlying "4S" geometry is developing. This class is dedicated specifically
 * to the Nucleus4SAgent -- it is based on the same geometry assumptions. It is
 * an utility class wrapped around four pieces of the
 * SpheresFunctions::CoordsUpdater that implements weighted displacement of any
 * texture dots based on their intersection(s) with the "4S" spheres (and the
 * depths of the dots inside these spheres).
 */
class TextureUpdater4S {
  ...
  private:
	/** a flag whose existence allows us to trigger the testNoOfSpheres() method
	   during this class construction; officially, however, this is a flag to
	   signal if the class is initialized */
	const bool objectIsReady;

	/** this method can be regarded as a container of a code that is executed as
...
/**
 * This class can update coordinates of the texture dots on-the-go as the
 * underlying "2+NS" geometry is developing. This class is designed for agents
 * that utilize an explicit body (polarity) axis and assume given two spheres
 * are attached to/are defining this axis. Technically, it is an utility class
 * wrapped around 2+N pieces of the SpheresFunctions::CoordsUpdater.
 *
 * The deal in "2+NS" geometry is that there is 2+N spheres and that some two
 * spheres (through their centres) define the current orientation of the agent,
 * which is then imposed on all spheres.
 */
class TextureUpdater2pNS {
  public:
	/** init the class with the current 2pNS geometry */
	TextureUpdater2pNS(const Spheres& geom,
	                   const int _sphereAtCentre = 0,
	...
	/** source of a vector -- this together defines the body main axis, the
	 * cell's polarity if you will */
	const int sphereAtCentre;
	/** end of a vector -- this together defines the body main axis, the cell's
	 * polarity if you will */
	const int sphereOnMainAxis;

	/** total number of spheres, including the special two above; this would
	   have been a template parameter if I were able to make this (otherwise
	   templated) class a friend of the Spheres class .. I had issues with the
	   template params */
	const int noOfSpheres;

	/** this method tracks the 2pNS geometry changes and updates, in accord, the
	   coordinates of the given list of texture dots */
	void updateTextureCoords(std::vector<Dot>& dots, const Spheres& newGeom);
  ...
  private:
	...
	/** aux array to be allocated once and not repeatedly in
	   ...
	   and complains about confusion with NucleusAgent::weights which is why the
	   attribute's name got crippled... */
  ...
  public:
	...
	TextureUpdaterNS(const Spheres& geom, int maxNoOfNeighs = 1)
	    : noOfSpheres(testAndGetNoOfSpheres(geom)),
	      neigWeightMatrix(noOfSpheres), prevCentre(noOfSpheres),
	      prevRadius(noOfSpheres), prevOrientation(noOfSpheres),
	...
	/** total number of spheres, including the special two; this would have been
	    a template parameter if I were able to make this (otherwise templated)
	   class a friend of the Spheres class .. I had issues with the template
	   params */
	const int noOfSpheres;

	/** extract "orientation" (into 'orientVec') of 'idx'-sphere in the given
	 * 'spheres' geometry */
	...
	/** For each sphere, all overlapping (that is, neighboring) spheres are
	   sorted first according to their radii, and according to the size of the
	   overlap of two spheres if both are of the same radius; larger means
	   earlier in the sorted list. Finally, 'maxNoOfNeighs' of spheres (or less
	   if that many is not available) are marked in the 'neigWeightMatrix', all
	   with weight 1. */
