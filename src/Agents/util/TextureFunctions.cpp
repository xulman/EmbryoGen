                         ...
                         const size_t noOfParticlesPerVoxel)
{
	...
	for (lineStart.y = geom.AABB.minCorner.y+gridFrom.y; lineStart.y < geom.AABB.maxCorner.y; lineStart.y += gridStep.y)
	{
		pos.from(lineStart);
	...
	lineStart.from(geom.AABB.minCorner) += gridFrom;
	for (; lineStart.z < geom.AABB.maxCorner.z; lineStart.z += gridStep.z)
	for (lineStart.x = geom.AABB.minCorner.x+gridFrom.x; lineStart.x < geom.AABB.maxCorner.x; lineStart.x += gridStep.x)
	{
		pos.from(lineStart);
