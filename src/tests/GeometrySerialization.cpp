...
// compile:
//
// g++ -o test -Wall GeometrySerialization.cpp  ../Geometries/*cpp -li3dalgo
// -li3dcore -Xlinker -defsym -Xlinker MAIN__=main
...
long noOfPixelDifferences(const i3d::Image3d<VT>& a,
                          const i3d::Image3d<VT>& b) {
...
void testImages(void) {
	...
	std::cout << "offset shift: "
	          << Deserialization::fromBuffer(buffer, got_fimg) << "\n";
	GetImageInfo(got_fimg);
	std::cout << "no of different pixels: "
	          << noOfPixelDifferences(fimg, got_fimg) << "\n";
	...
	std::cout << "offset shift: " << Serialization::toBuffer(gimg, bufferB)
	          << "\n";
	std::cout << "offset shift: "
	          << Deserialization::fromBuffer(bufferB, got_gimg) << "\n";
	GetImageInfo(got_gimg);
	std::cout << "no of different pixels: "
	          << noOfPixelDifferences(gimg, got_gimg) << "\n";
}
...
void GetImageInfo(const i3d::Image3d<VOXEL>& image) {
	...
	cout << "Image size um: "
	     << i3d::PixelsToMicrons(image.GetSize(), image.GetResolution())
	     << " um\n";
	cout << "Image resolution: " << image.GetResolution().GetRes()
	     << " px/um\n";
	cout << "Image offset: " << image.GetOffset() << " um\n";
	size_t ImageSize = image.GetImageSize() * sizeof(VOXEL);
	cout << "Image size: " << ImageSize << " B\n";
	if (ImageSize > (size_t(1) << 30))
		cout << "Image size human: " << (ImageSize >> 30) << " GB\n";
	else if (ImageSize > (size_t(1) << 20))
		cout << "Image size human: " << (ImageSize >> 20) << " MB\n";
	else if (ImageSize > (size_t(1) << 10))
		cout << "Image size human: " << (ImageSize >> 10) << " kB\n";
	else
