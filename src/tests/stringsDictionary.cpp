...
class DictionaryBroadcaster : public StringsDictionary {
  public:
	void StartBroadcastFromMeToNetwork() {
		for (const auto& i : theseShouldBeBroadcast()) {
			std::cout << "This is travelling via network: >>" << i.second
			          << "<<\n";
			// NB: no need to send i.first == ID == hash, as receiver can
			// calculate it too,
	...
	void StopBroadcastFromMeToNetwork() {
		...
		//...and pass token on some other FO to start broadcasting its recent
		// additions into its Dictionary
	}

	void ProcessItemFromNetwork(const StringsDictionary& remoteSource) {
		// normally the data arrives from MPI,
		// here, we fake this communication by reading out new stuff from
		// remoteSource
		//
		// actually, the remoteSource.theseShouldBeBroadcast() should be
		// considered in this fake but we cannot reach it, luckily its
		// showNewDictionary() gives the same data
...
int main(void) {
	try {
		...
		hashedString agent1_1(
		    "nucleus 1"); // create new agentType for whatever reason
		...
		hashedString veryLong(
		    "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
		    "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
		    "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
		    "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAFFEEDDC260");
		...
		// FO 1, some testing:
		std::cout << "My  agent1: " << agent1_1.getString() << " of hash "
		          << agent1_1.getHash() << "\n";
		std::cout << "Dictionary: "
		          << d1.translateIdToString(agent1_1.getHash()) << "\n\n";

		std::cout << "My  agent2: " << agent1_2.getString() << " of hash "
		          << agent1_2.getHash() << "\n";
		std::cout << "Dictionary: "
		          << d1.translateIdToString(agent1_2.getHash()) << "\n\n";
