...
void testInterpolation(void) {
	const DivisionModels2S<5, 5> sample("data/divisionModelsForEmbryoGen.dat",
	                                    3.0f);
	...
	for (float t = -17; t < 0; t += 0.5)
		std::cout << t << ": " << m.getMotherRadius(t, 0) << "-"
		          << m.getMotherDist(t, 0) << "-" << m.getMotherRadius(t, 1)
		          << "\n";
	std::cout << "\n\n";

	std::cout << "# evolution for daughter1 from model ID " << modelId << ":\n";
	for (float t = 0; t < 15; t += 0.5)
		std::cout << t << ": " << m.getDaughterRadius(t, 0, 0) << "-"
		          << m.getDaughterDist(t, 0, 0) << "-"
		          << m.getDaughterRadius(t, 0, 1) << "\n";
	std::cout << "\n\n";

	std::cout << "# evolution for daughter2 from model ID " << modelId << ":\n";
	for (float t = 0; t < 15; t += 0.5)
		std::cout << t << ": " << m.getDaughterRadius(t, 1, 0) << "-"
		          << m.getDaughterDist(t, 1, 0) << "-"
		          << m.getDaughterRadius(t, 1, 1) << "\n";
}
...
void testWeights(void) {
	...
	sample.printWeightsWithModelsVertically<20, fromThisNoOfRandomModels>(
	    weights);
}
