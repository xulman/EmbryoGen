...
void showGeom(DisplayUnit& du,
              const Spheres* srcGeom,
              const Spheres* expandedGeom) {
	int i = 0;
	...
	std::cout << "showing " << expandedGeom->getNoOfSpheres()
	          << " expanded spheres\n";
	for (i = 0; i < expandedGeom->getNoOfSpheres(); ++i) {
		du.DrawPoint(10 + i, expandedGeom->getCentres()[i],
		             expandedGeom->getRadii()[i], 2);
		// du.DrawVector(i,futureGeometry.getCentres()[i],orientVec,0);
	}
}
...
void mainForLinkedSpheres(DisplayUnit& du) {
	...
	twoS.updateCentre(
	    0, Vector3d<float>(
	           -9.f, GetRandomUniform(-maxAxisDev * sDist, +maxAxisDev * sDist),
	...
	SpheresFunctions::LinkedSpheres<float> builder(twoS,
	...
	builder.resetAllAzimuthsToExtrusions(
	    minAngle, stepAngle, maxAngle,
	    [](float f) { return 10 * (sin(f * M_PI) + 0.3 * sin(f * 4 * M_PI)); });
	...
	float myAng = M_PI;
	builder.addOrChangeAzimuth(
	    myAng,
	    SpheresFunctions::LinkedSpheres<float>::AzimuthDrivenPositionExtruder(
	        myAng, builder, [](float) { return 5.f; }),
	    builder.defaultRadiusNoChg, 5);

	// explicitly placed lines with specialities
	builder.addOrChangeAzimuth(
	    myAng + 000.1, builder.defaultPosNoAdjustment,
	    [](float, float f) { return 2.f - 2.f * std::abs(f - 0.5f); }, 4);
	...
	builder.printPlan();
	report::message(
	    fmt::format("necessary cnt: {}", builder.getNoOfNecessarySpheres()));
	...
	int vecID = 1000;
	du.DrawVector(vecID++, centre, builder.getBasalSideAxis(),
	              1); // basal vector red
	du.DrawVector(vecID++, centre, builder.getRectifiedBasalDir(),
	              5); // rectified basal vector magenta
	du.DrawVector(vecID++, centre, builder.getAux3rdAxis(), 4); // aux 3rd cyan
	du.DrawLine(vecID++, twoS.getCentres()[0], twoS.getCentres()[1],
	            1); // main axis in red
	...
	while (key != 'a') {
		...
		if (key == 'p') {
			twoS.updateCentre(1, refCentre +
			                         Vector3d<float>(0, 4 * sin(0.4 * cnt), 0));
		}
		if (key == 'u') {
...
void mainForSimplyLinkedSpheres(DisplayUnit& du) {
	...
	SpheresFunctions::LinkedSpheres<float> builder(twoS,
	                                               Vector3d<float>(0, 1, 0));
	builder.addOrChangeAzimuth(0, builder.defaultPosNoAdjustment,
	                           builder.defaultRadiusNoChg, 4);
