...
SceneControls& provideSceneControls_withOwnConstants() {
	...
	// this line demonstrates that SceneControls.constants are indeed constants
	// because any changes in source object since now do not affect the one
	// inside *sc
...
class provideSceneControls_withOwnUpdater : public SceneControls {
  public:
	void updateControls(const float currTime) override {
		report::message(
		    fmt::format("my own special updater at time {}", currTime));
	}
};

class provideSceneControls_withOwnConstantsAndUpdater : public SceneControls {
  public:
	provideSceneControls_withOwnConstantsAndUpdater(Constants& c)
	    : SceneControls(c) {}
...
void testCreatingAndSettingScenario::initializeAgents(FrontOfficer* fo,
                                                      int fractionNumber,
                                                      int noOfAllFractions) {
...
void testBasicScenario::initializeScene() { report::message("init Scene"); }

void testBasicScenario::initializeAgents(FrontOfficer* fo,
                                         int fractionNumber,
                                         int noOfAllFractions) {
	report::message(
	    fmt::format("add agents {}/{}", fractionNumber, noOfAllFractions));
}
...
class testExtendedScenario : public testBasicScenario {
  public:
	void testWork() {
		report::message(
		    fmt::format("constants.incrTime = {}", params.constants.incrTime));
		report::message(fmt::format("producingMasks? = {}",
		                            params.isProducingOutput(params.imgMask)));
		report::message(fmt::format("producingMasks? = {}",
		                            params.imagesSaving_isEnabledForImgMask()));
...
int main(int argc, char** argv) {
	...
	std::cout << " preInit: incrTime = "
	          << s.seeCurrentControls().constants.incrTime << "\n";
	std::cout << " preInit: expoTime = "
	          << s.seeCurrentControls().constants.expoTime << "\n";
	s.initializeScene();
	s.updateScene(99);
	std::cout << "postInit: incrTime = "
	          << s.seeCurrentControls().constants.incrTime << "\n";
	std::cout << "postInit: expoTime = "
	          << s.seeCurrentControls().constants.expoTime << "\n";
	...
	report::message(fmt::format(
	    "producingMasks? = " << es_sc.isProducingOutput(es_sc.imgMask)));
}
