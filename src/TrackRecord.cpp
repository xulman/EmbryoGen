...
void TrackRecords::injectToFF(const float timeFrom,
                              const float timeTo,
                              FlowField<float>& FF,
	...
	//
	// assure sigmaSq is never zero and is mega-large (=100px) where it would
	// have stayed zero
	if (halfBoxSize.x == 0)
		sigmaSq.x = 10000;
	if (halfBoxSize.y == 0)
	...
	report::debugMessage(fmt::format("halfBoxSize = {}, sigmaSq = {}",
	                                 toString(halfBoxSize), toString(sigmaSq)));

	// positions in microns (in real world coords) at a track at the given two
	// times
	Coord3d<float> A, B;
	...
	for (const auto id : knownTracks)
		if (getPositionAlongTrack(timeFrom, id, A) &&
			...
			for (z = posI.z - halfBoxSize.z; z <= posI.z + halfBoxSize.z; ++z)
				for (y = posI.y - halfBoxSize.y; y <= posI.y + halfBoxSize.y;
				     ++y) {
					float attenuation =
