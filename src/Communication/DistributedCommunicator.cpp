...
int MPI_Communicator::init(int argc, char** argv) {
	...
	if (out_model != MPI_THREAD_MULTIPLE) {
		report::error("Could not initialize MPI with thread support");
	...
	MPI_Aint v3d_displ[] = {offsetof(Vector3d<float>, x),
	                        offsetof(Vector3d<float>, y),
	                        offsetof(Vector3d<float>, z)};
	MPI_Datatype v3d_types[] = {MPI_FLOAT, MPI_FLOAT, MPI_FLOAT};

	MPI_Type_create_struct(v3d_count, v3d_blocks, v3d_displ, v3d_types,
	                       &MPI_VECTOR3D);
	MPI_Type_commit(&MPI_VECTOR3D);

	const int aabb_count = 5;
	int aabb_blocks[] = {1, 1, 1, 1, 1};
	MPI_Aint aabb_displ[] = {offsetof(t_aabb, minCorner),
	                         offsetof(t_aabb, maxCorner), offsetof(t_aabb, id),
	                         offsetof(t_aabb, version),
	                         offsetof(t_aabb, atype)};
	MPI_Datatype aabb_types[] = {MPI_VECTOR3D, MPI_VECTOR3D, MPI_INT, MPI_INT,
	                             MPI_UINT64_T};

	MPI_Type_create_struct(aabb_count, aabb_blocks, aabb_displ, aabb_types,
	                       &MPI_AABB);
	state = MPI_Type_commit(&MPI_AABB);

	const int agentt_count = 2;
	int agentt_blocks[] = {1, StringsImprintSize};
	MPI_Aint agentt_displ[] = {offsetof(t_hashed_str, hash),
	                           offsetof(t_hashed_str, value)};
	MPI_Datatype agentt_types[] = {MPI_UNSIGNED_LONG_LONG, MPI_CHAR};
	MPI_Type_create_struct(agentt_count, agentt_blocks, agentt_displ,
	                       agentt_types, &MPI_AGENTTYPE);
	...
	//MPI_Comm_split(MPI_COMM_WORLD, (instance_ID != *director_id), instance_ID,
	&aabb_comm);
	...
	*/
	...
	MPI_Comm_dup(/*MPI_COMM_WORLD*/ aabb_comm, &type_comm); // TBD: Or
	                                                        // aabb_comm?
...
bool MPI_Communicator::detectMPIMessage(MPI_Comm comm,
                                        int peer,
                                        e_comm_tags& tag,
                                        bool async) {
	int flag;
...
bool MPI_Communicator::receiveAndProcessDirectorMessage(void* buffer,
                                                        int& recv_size,
                                                        e_comm_tags& tag) {
	//	MPI_Status status;
	bool result;
...
bool MPI_Communicator::receiveDirectorMessage(void* buffer,
                                              int& recv_size,
                                              e_comm_tags& tag,
                                              bool /*async*/) {
	MPI_Status status;
	...
	state = receiveMPIMessage(comm, buffer, recv_size, tagMap(tag), &status,
	                          did, tag);
	if (state == MPI_SUCCESS) {
		tag = (e_comm_tags)status.MPI_TAG;
		return true;
...
int MPI_Communicator::receiveMPIMessage(MPI_Comm comm,
                                        void* data,
                                        int& items,
...
bool MPI_Communicator::receiveFOMessage(void* buffer,
                                        int& recv_size,
                                        int& instance_ID,
                                        e_comm_tags& tag) {
	MPI_Status status;
...
void MPI_Communicator::startNewAgent(
...
void MPI_Communicator::startNewDaughterAgent(const int childID,
	...
	sendDirector(buffer, sizeof(buffer) / sizeof(int),
	             e_comm_tags::update_parent);
	receiveDirectorACK();
}

void MPI_Communicator::setAgentsDetailedDrawingMode(int FO,
                                                    int agentID,
                                                    bool state) {
	int buffer[] = {agentID, state};
	sendFO(buffer, sizeof(buffer) / sizeof(int), FO,
	       e_comm_tags::set_detailed_drawing);
	receiveFOACK(FO);
}

void MPI_Communicator::setAgentsDetailedReportingMode(int FO,
                                                      int agentID,
                                                      bool state) {
	int buffer[] = {agentID, state};
	sendFO(buffer, sizeof(buffer) / sizeof(int), FO,
	       e_comm_tags::set_detailed_reporting);
	receiveFOACK(FO);
}
...
size_t MPI_Communicator::receiveRenderedFrame(int /*fromFO*/,
                                              int /*slice_size*/,
                                              int /*slices*/) {
...
void MPI_Communicator::mergeImages(int FO,
                                   int slice_size,
                                   int slices,
                                   unsigned short* maskPixelBuffer,
                                   float* phantomBuffer,
                                   float* opticsBuffer) {
	unsigned short* mask_add =
	    (unsigned short*)malloc((slice_size + 1) * sizeof(unsigned short));
...
report::debugMessage(fmt::format("From #{} to #{} merge images plane size {} slices {}" , instance_ID, FO, slice_size, slices));
	for (int slice = 0; slice < slices; slice++) {
		...
		unsigned short* mask_start =
		    maskPixelBuffer + slice * sizeof(unsigned short);
...
report::debugMessage(fmt::format("Receive at FO #{} slice {}" , instance_ID, slice));
			...
			if (phantomBuffer) {
				receiveMPIMessage(image_comm, phantom_add, received_slice_size,
				                  tagMap(rimg), MPI_STATUSES_IGNORE,
				                  received_from, rimg);
			}
			if (opticsBuffer) {
				receiveMPIMessage(image_comm, optics_add, received_slice_size,
				                  tagMap(rimg), MPI_STATUSES_IGNORE,
				                  received_from, rimg);
			}
			...
			for (int idx = 0; idx < slice_size; idx++) {
				...
				if (opticsBuffer) {
					optics_start[idx] += optics_add[idx];
				} // Probably Not correct, should be mean/median later?
			}
		}
...
report::debugMessage(fmt::format("Send from #{} to #{} slice {}" , instance_ID, FO, slice));
		...
		if (maskPixelBuffer) {
			sendMPIMessage(image_comm, mask_start, slice_size,
			               tagMap(e_comm_tags::mask_data), FO,
			               e_comm_tags::mask_data);
		}
		if (phantomBuffer) {
			sendMPIMessage(image_comm, phantom_start, slice_size,
			               tagMap(e_comm_tags::float_image_data), FO,
			               e_comm_tags::float_image_data);
		}
		if (opticsBuffer) {
			sendMPIMessage(image_comm, optics_start, slice_size,
			               tagMap(e_comm_tags::float_image_data), FO,
			               e_comm_tags::float_image_data);
		}
...
report::debugMessage(fmt::format("Receive at Director{} slice {}" , instance_ID, slice));
			...
			if (phantomBuffer) {
				receiveMPIMessage(image_comm, phantom_start,
				                  received_slice_size, tagMap(rimg),
				                  MPI_STATUSES_IGNORE, received_from, rimg);
			}
			if (opticsBuffer) {
				receiveMPIMessage(image_comm, optics_start, received_slice_size,
				                  tagMap(rimg), MPI_STATUSES_IGNORE,
				                  received_from, rimg);
			}
			// assert(received_slice_size == slice_size && received_from ==
...
void MPI_Communicator::sendNextID(int id) {
	...
	sendLastFO(
	    &id, 1,
	    tag); /* MISSING Agent ID, for now solved with this variable !!!*/
}
