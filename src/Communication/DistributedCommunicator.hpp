...
#define StringsImprintSize                                                     \
	256 // This should be moved to some common header file with strings
...
#define DIRECTOR_RECV_MAX                                                      \
	(1 << 20)             // Receive buffer for director-only communication
...
class DistributedCommunicator {
  public:
	...
	virtual bool receiveAndProcessDirectorMessage(void* buffer,
	                                              int& recv_size,
	                                              e_comm_tags& tag) = 0;
	virtual bool receiveDirectorMessage(void* buffer,
	                                    int& recv_size,
	                                    e_comm_tags& tag,
	                                    bool async = true) = 0;
	virtual size_t
	receiveRenderedFrame(int fromFO, int slice_size, int slices) = 0;

	inline int sendACKtoDirector() {
		char id = 0;
		return sendDirector(
		    &id, 0, e_comm_tags::ACK); // We could also store last tag and send
		                               // it back instead of ACK
	}

	inline void receiveDirectorACK() {
		e_comm_tags tag =
		    e_comm_tags::ACK; // Should we use ::unspecified instead?
	...
	virtual bool receiveFOMessage(
	    void* buffer,
	    int& recv_size,
	    int& instance_ID,
	    e_comm_tags& tag) = 0; // Single reception step, for response messages
	virtual int
	sendFO(void* data, int count, int instance_ID, e_comm_tags tag) = 0;
	...
	/*		inline int sendACKtoLastFO() { //Probably broken
	            char id = 0;
	            return sendLastFO(&id, 0, e_comm_tags::ACK); // We could also
	   store last tag and send it back instead of ACK
	        }
	*/
	...
	inline virtual int
	sendPrevFO(void* data,
	           int count,
	           e_comm_tags tag) { // Is this method needed at all?
		return sendFO(data, count, (instance_ID + instances - 1) % instances,
		              tag);
	}

	inline int receiveAndProcessNextFOMessages(
	    e_comm_tags tag = e_comm_tags::unspecified) {
		int toFO = (instance_ID + 1) % instances;
		return receiveFOMessage(director_buffer, director_buffer_size, toFO,
		                        tag);
	}

	inline int receiveAndProcessPrevFOMessages(
	    e_comm_tags tag = e_comm_tags::unspecified) {
	...
	/*** Broadcast channel ***/
	virtual bool
	sendBroadcast(void* data, int count, int sender_id, e_comm_tags tag) = 0;
	virtual bool receiveBroadcast(void* data,
	                              int& count,
	                              int sender_id,
	                              e_comm_tags tag) = 0;
	...
	/*{
	    return (instance_ID << shift) | (++internal_agent_ID); //Recalculate
	better to take into account milions of cells!!!
	}*/

	virtual void
	startNewAgent(const int newAgentID,
	              const int associatedFO,
	              const bool wantsToAppearInCTCtracksTXTfile = true) = 0;
	virtual void closeAgent(const int agentID, const int associatedFO) = 0;
	virtual void startNewDaughterAgent(const int childID,
	...
	virtual void
	setAgentsDetailedDrawingMode(int FO, int agentID, bool state) = 0;
	virtual void
	setAgentsDetailedReportingMode(int FO, int agentID, bool state) = 0;
	...
	virtual void mergeImages(int FO,
	                         int slice_size,
	                         int slices,
	                         unsigned short* maskPixelBuffer,
	                         float* phantomBuffer,
	                         float* opticsBuffer) = 0;
  ...
  protected:
	...
	// Unused now: int shift; // Shift ID by given number of bits, to reduce
	// communication for getNextAvailAgentID, maybe rework later
	static e_comm_tags messageType; // Message type enum for sending/receiving
	                                // individual distributed messages
...
class MPI_Communicator : public DistributedCommunicator {
  public:
	...
	virtual void
	setAgentsDetailedReportingMode(int FO, int agentID, bool state);
	...
	virtual int sendDirector(void* data, int count, e_comm_tags tag) {
		// MPI_Comm comm = (tag ==
		// e_comm_tags::ACK)?MPI_COMM_WORLD:director_comm;
	...
	virtual int
	sendFO(void* data, int count, int instance_ID, e_comm_tags tag) {
		// MPI_Comm comm = (tag ==
		// e_comm_tags::ACK)?MPI_COMM_WORLD:director_comm;
		MPI_Comm comm = tagCommMap(tag, director_comm);
	...
	virtual bool receiveAndProcessDirectorMessage(
	    void* buffer,
	    int& recv_size,
	    e_comm_tags& tag); // Probably not needed to run in cycle if
	                       // notifications separate, only director needs cycle
	virtual bool receiveDirectorMessage(
	    void* buffer,
	    int& recv_size,
	    e_comm_tags& tag,
	    bool async =
	        true); // Single reception step, for request-response messages
	...
	virtual bool receiveFOMessage(
	    ...
	    int& recv_size,
	    int& instance_ID,
	    e_comm_tags& tag); // Single reception step, for response messages
  ...
  protected:
	...
	inline void debugMPIComm(const char* what,
	                         MPI_Comm comm,
	                         int items,
		...
		report::message(fmt::format("{} MPI message at: {} Via: {} Peer: {} "
		                            "Items in message: {} Tag: {}",
		                            what, instance_ID, cname, peer, items,
		                            tagName(tag)));
	}
#else
	...
	inline int sendMPIMessage(MPI_Comm comm,
	                          void* data,
	                          int items,
	...
	int sendMPIBroadcast(
	    MPI_Comm comm,
	    void* data,
	    int items,
	    MPI_Datatype datatype) { // AABB from each FO to all as an example
		debugMPIComm("Broadcast", comm, items, instance_ID,
		             e_comm_tags::unspecified);
		return MPI_Bcast(data, items, datatype, instance_ID, comm);
	}

	int receiveMPIMessage(MPI_Comm comm,
	                      void* data,
	                      int& items,
	                      MPI_Datatype datatype,
	                      MPI_Status* status,
	...
	inline MPI_Datatype tagMap(e_comm_tags tag) {
		...
		case e_comm_tags::float_image_data:
			return MPI_FLOAT;
		case e_comm_tags::new_type: // byte array or a special MPI structure? -
		                            // UINT64 + 256*Char !!!
			/*return MPI_CHAR;*/
			return MPI_AGENTTYPE;
		// case e_comm_tags:: : //Image size -> define image type dynamically
		// after start, each frame!!! case e_comm_tags::set_detailed_drawing:
		// case e_comm_tags::set_debug: //could be also MPI_C_BOOL
		default: // shadow_copy, render frame, // Conversions would be slow?
			return MPI_BYTE;
		}
	}
	...
	inline MPI_Comm tagCommMap(e_comm_tags tag, MPI_Comm def = MPI_COMM_WORLD) {
		...
		case e_comm_tags::barrier:
			return barrier_comm;
		default: // Every other message should be passed by async director
		         // communication - but it may break something
			return def;
			//					return director_comm;
		}
	}

	bool detectMPIMessage(MPI_Comm comm,
	                      int peer,
	                      e_comm_tags& tag,
	                      bool async = true);
	...
	/*** Broadcast channel ***/
	virtual bool
	sendBroadcast(void* data, int items, int sender_id, e_comm_tags tag) {
	...
	virtual bool
	receiveBroadcast(void* data, int& items, int sender_id, e_comm_tags tag) {
		debugMPIComm("Receive Broadcast", tagCommMap(tag), items, sender_id,
		             tag);
		return MPI_Bcast(data, items, tagMap(tag), sender_id, tagCommMap(tag));
	}

	// int receiveAndProcessMPIBroadcasts(e_comm_tags tag =
	// e_comm_tags::unspecified);

	// Frame rendering not necessary to wait for token
	// for with ID to distunguish send/receive for AABB? Token passing better
	// (scene with more independent part)
	...
	MPI_Comm token_comm; // Channel to synchronize FO according to token passing
	                     // description
