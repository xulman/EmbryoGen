...
void FrontOfficer::request_startNewAgent(
	...
	communicator->startNewAgent(newAgentID, associatedFO,
...
void FrontOfficer::waitFor_publishAgentsAABBs() {
	report::debugMessage(
	    fmt::format("FO #{} is running AABB reporting cycle with local size {}",
	                this->ID, agents.size()));
	t_aabb* sentAABBs;
	for (int i = 1; i <= FOsCount; i++) {
		if (i == ID) {
			int aabb_count =
			    (int)agents.size(); // getSizeOfAABBsList(); is not returning
			                        // correct results! (WHY? -> Because the
			                        // AABB list is empty)
			/* TBD: In future, will local AABB count in each FO always fit into
			 * integer? Otherwise we have a problem */
			int j = 0;
			...
			for (auto ag : agents) {
				...
				// report::message(fmt::format("Agent #{} is running AABB
				// reporting cycle with local size {}" , j, agents.size()));
			...
			communicator->sendBroadcast(sentAABBs, aabb_count, i,
			...
			report::debugMessage(fmt::format(
			    "Receive {} AABBs at FO#{} from FO #{}", aabb_count, ID, i));
			sentAABBs = (t_aabb*)malloc(sizeof(t_aabb) * (aabb_count + 1));
			...
			for (int j = 0; j < aabb_count; j++) {
				...
				/*DEBUG_REPORT("FO #" << ID << ", AABB #" << j
				  << " Min: (" << sentAABBs[j].minCorner.x << "," <<
				sentAABBs[j].minCorner.y << "," << sentAABBs[j].minCorner.z <<
				")"
				  << " Max: (" << sentAABBs[j].maxCorner.x << "," <<
				sentAABBs[j].maxCorner.y << "," << sentAABBs[j].maxCorner.z <<
				")"
				);*/
				AABBs.back().ID = sentAABBs[j].id;
				AABBs.back().nameID = sentAABBs[j].atype;
				agentsAndBroadcastGeomVersions[sentAABBs[j].id] =
				    sentAABBs[j].version;
				/*DEBUG_REPORT("FO #" << ID << ", Agent #" << j << " ID: " <<
				sentAABBs[j].id
				  << " Name ID: " << sentAABBs[j].atype <<  ", Geometry version
				" <<sentAABBs[j].version
				);*/
	...
	report::debugMessage(fmt::format(
	    "FO #{} has finished AABB reporting cycle with global size {}",
	    this->ID, (AABBs.size() + agents.size())));
...
void FrontOfficer::broadcast_AABBofAgent(const ShadowAgent& ag) {
	// Since we already do a global broadcast of all AABBs at once, we will use
	// this method to update the AABB list, which is not done in main code
	// (WHY?)

	report::debugMessage(fmt::format(
	    "FO #{} is adding AABB of shadow agent ID {}", this->ID, ag.ID));
	int aid = ag.getID();
...
void FrontOfficer::respond_AABBofAgent() {
	// This method would miss important parameters and is executed in global
	// receiving stage so it is dummy
}
...
void FrontOfficer::broadcast_newAgentsTypes() {
	int new_dict_count = (int)agentsTypesDictionary
	                         .howManyShouldBeBroadcast() /*Integer limit !!!*/,
	    total_cnt = 0;
	report::debugMessage(fmt::format(
	    "FO #{} is running New Agent Type reporting cycle with local size {}",
	    this->ID, new_dict_count));
	t_hashed_str* sentTypes;
	/*	char * sentTypes;*/
	for (int i = 1; i <= FOsCount; i++) {
		if (i == ID) {
			total_cnt += new_dict_count =
			    (int)agentsTypesDictionary.howManyShouldBeBroadcast();
			int j = 0;
			report::debugMessage(fmt::format(
			    "New dictionary count From FO#{}: {}", i, new_dict_count));
			communicator->sendBroadcast(&new_dict_count, 1, i,
			                            e_comm_tags::count_new_type);
			// sentTypes = new t_hashed_str[new_dict_count];
			sentTypes =
			    (t_hashed_str*)calloc(sizeof(t_hashed_str), new_dict_count + 1);
			/*			new_dict_count *= StringsImprintSize;
			            sentTypes = new char[new_dict_count];*/
			for (const auto& dItem :
			     agentsTypesDictionary.theseShouldBeBroadcast()) {
				sentTypes[j].hash = dItem.first;
				hashedString::printIntoBuffer(dItem.second, sentTypes[j].value,
				                              StringsImprintSize);
				// DEBUG_REPORT("Sending Type " <<  sentTypes[j].value << " with
				// hash " << dItem.first << " from FO#" << i);
				/*hashedString::printIntoBuffer(dItem.second,
				 * sentTypes+j*StringsImprintSize,StringsImprintSize);*/
				j++;
			}
			communicator->sendBroadcast(sentTypes, new_dict_count, i,
			                            e_comm_tags::new_type);
			free(sentTypes);
			//			delete [] sentTypes;
		} else {
			int cnt = 1;
			communicator->receiveBroadcast(&new_dict_count, cnt, i,
			                               e_comm_tags::count_new_type);
			assert(cnt == 1);
			total_cnt += new_dict_count;
			// sentTypes = new t_hashed_str[new_dict_count];
			...
			/*			new_dict_count *= StringsImprintSize;
			            sentTypes = new char[new_dict_count];*/
			communicator->receiveBroadcast(sentTypes, new_dict_count, i,
			                               e_comm_tags::new_type);
			for (int j = 0; j < new_dict_count; j++) {
				/*hashedString * hs = new
				hashedString(sentTypes+j*StringsImprintSize);
				agentsTypesDictionary.enlistTheIncomingItem(hs->getHash(),
				hs->getString());*/
				agentsTypesDictionary.enlistTheIncomingItem(sentTypes[j].hash,
				                                            sentTypes[j].value);
				report::debugMessage(
				    fmt::format("Receiving Type {} with hash {} at FO#{}",
				                sentTypes[j].value, sentTypes[j].hash, ID));
			}
			free(sentTypes);
			//			delete [] sentTypes;
		}
	...
	// DEBUG_REPORT("Checking hash for nucleus 2015 @ 4,2:" <<
	// agentsTypesDictionary.translateIdToString(5627021567199719035l));
#ifdef DISTRIBUTED_DEBUG
	agentsTypesDictionary.printKnownDictionary();
#endif /*DISTRIBUTED_DEBUG*/
	communicator->waitFor_publishAgentsAABBs();
	report::debugMessage(fmt::format("FO #{} has finished New Agent Type "
...
ShadowAgent* FrontOfficer::request_ShadowAgentCopy(const int agentID,
	...
	report::debugMessage(fmt::format(
	    "Received shadow copy info at FO #{}: ID={}, Type={}, Geom Type={}", ID,
	    param_buff[0], param_buff[2], param_buff[3]));
...
void FrontOfficer::respond_ShadowAgentCopy(const int agentID) {
	...
	const Geometry& sendBackGeom =
	    aaRef.getGeometry(); // Correct geometry -> simplified
	...
	size_t param_buff[4] = {(size_t)sendBackAgentID, (size_t)geom_size,
	                        sendBackAgentType, (size_t)geom_type};
	int cnt = 4; // sizeof(param_buff) / sizeof(long*);

	report::debugMessage(fmt::format(
	    "Sent shadow copy info at FO #{}: ID={}, Type={}, Geom Type={}", ID,
	    param_buff[0], param_buff[2], param_buff[3]));
	...
	communicator->sendFO(buffer_to_send.get(), (int)geom_size, foID,
	                     e_comm_tags::shadow_copy_data);
}
...
void FrontOfficer::respond_Loop() {
	...
	/*communicator->sendDirector(buffer,0, e_comm_tags::next_stage); //Start the
director -> FO sending report::message(fmt::format("Unblock level {} in FO {}" ,
unblock_lvl, ID)); if (unblock_lvl <= 0) { report::message(fmt::format("Waiting
for sync in FO {}" , ID)); tag=e_comm_tags::unblock_FO;
	...
	do {
		...
		if ((tag = communicator->detectFOMessage(
		         false /*we are runnung synchronously in the thread*/)) < 0) {
		...
		if (tag == e_comm_tags::ACK || tag == e_comm_tags::unblock_FO) {
			report::message(fmt::format(
			    "ACK/Unblock on Director Communicator on FO #{}", ID));
			std::this_thread::sleep_for((std::chrono::milliseconds)10);
			continue;
		}
		switch (tag) {
		...
		/*case e_comm_tags::next_stage:
			...
			break;*/
		case e_comm_tags::set_debug: // Receive and send to following FO to
		                             // distribute it
			communicator->receiveFOMessage(buffer, items, instance, tag);
		...
		default:
			report::message(
			    fmt::format("Unprocessed communication tag {} on FO {}",
			                communicator->tagName(tag), ID));
			communicator->receiveFOMessage(buffer, items, instance, tag);
			break;
		}

	} while (!this->finished &&
	         !communicator->isFinished()); //(tag != e_comm_tags::next_stage);
	report::message(
	    fmt::format("Ending detection loop in FO {} and waiting for sync", ID));
	// communicator->waitSync(); //waitFor_publishAgentsAABBs();
}

void FrontOfficer::waitHereUntilEveryoneIsHereToo() // Will this work without
                                                    // specification of stage as
                                                    // an argument?
{
	report::debugMessage(
	    fmt::format("Wait Here Until Everyone Is Here Too FO #{}", ID));
...
void FrontOfficer::waitFor_renderNextFrame(const int FOsID) {
	...
	report::debugMessage(
	    fmt::format("Request image merging from FO #{} to FO #{}", ID, FOsID));
	report::debugMessage(
...
void FrontOfficer::respond_setRenderingDebug() {
	// Dummy method as there is no way to pass the parameter with rendering
	// debug flag
	report::debugMessage(fmt::format("FO #{} setRendering debug to {}",
	                                 this->ID, renderingDebug));
