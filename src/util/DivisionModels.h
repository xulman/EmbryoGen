...
struct DivisionModel
{
	...
	float getDaughterRadius(const float time, const int daughterNo, const int sphereNo) const
	{
#ifdef DEBUG
		if (time < 0)
	...
	float getDaughterDist(const float time, const int daughterNo, const int sphereNo) const
	{
#ifdef DEBUG
		if (time < 0)
			REPORT("WARNING: Daughter's configuration not available for negative time.");
...
private:
	template <int T, int I> //T = no. of Time points, I = no. of Items (Spheres) per time point
	float getArrayValue(const float time, const int itemIdx,
	                    const float times[T], const float values[T][I]) const
	{
#ifdef DEBUG
		...
		if (time <= times[0]) return values[0][itemIdx];

		int i=1;
		while (i < T && time > times[i]) ++i;
		if (i < T)
			return getInterpolatedValue(times[i-1],time,times[i], values[i-1][itemIdx],values[i][itemIdx]);
		else
	...
	float getInterpolatedValue(const float posA, const float pos, const float posB,
...
public:
	...
	/** friendly front-end method to obtain mixed model from the given number of original
	    models, the mixing is done with gaussian shaped weights where the mean of the gaussian
	                             ...
	                             const DivisionModel* models[M]) const
	{
		...
		for (int s=0; s < N; ++s)
		{
			...
			for (int n=1; n < M; ++n)
				m.Mradii[t][s] += weights[t][n] * models[n]->getMotherRadius(m.Mtimes[t],s);
		}
		...
		for (int s=0; s < N-1; ++s)
		{
			...
			for (int n=1; n < M; ++n)
				m.Mdists[t][s] += weights[t][n] * models[n]->getMotherDist(m.Mtimes[t],s);
		}
		...
		for (int s=0; s < N; ++s)
		{
			m.Dradii[d][t][s] = weights[t][0] * models[0]->Dradii[d][t][s];
			for (int n=1; n < M; ++n)
				m.Dradii[d][t][s] += weights[t][n] * models[n]->getDaughterRadius(m.Dtimes[d][t],d,s);
		}
		...
		for (int s=0; s < N-1; ++s)
		{
			m.Ddists[d][t][s] = weights[t][0] * models[0]->Ddists[d][t][s];
			for (int n=1; n < M; ++n)
				m.Ddists[d][t][s] += weights[t][n] * models[n]->getDaughterDist(m.Dtimes[d][t],d,s);
		}
	}
	...
	void setSineShiftingGaussWeights(float w[T][M]) const
	{
		...
		for (int t=0; t < T; ++t)
		{
			const float pos = (M-1.f) * (0.5f*std::cos(6.28 * t/T) +0.5f);
			setGaussWeights<M>(w[t],pos);
		}
	}

	template <int T, int M> //T = no. of Time points, M = no. of models to be mixed
	void setSineShiftingBoxWeights(float w[T][M], const float boxAmpl = 3) const
	{
		//it must hold Sum w[fixed_t][*] = 1
		for (int t=0; t < T; ++t)
		{
	...
	template <int M>
	void setBoxWeights(float w[M], const float boxPos, const float boxMagnitude) const
	{
	...
	void readOneDataBlock(std::ifstream& f, DivisionModel& dm) const
	{
		...
		//number of processed lines for every part of the model: 0 (mother) + D daughters
		int lines[D+1];
		for (int d=0; d < D+1; ++d) lines[d] = 0;
		...
		while (firstChar != std::ifstream::traits_type::eof() && std::isdigit(firstChar))
		{
			...
			f >> r;
			if (role == 0) dm.Mradii[lines[0]][N-1] = r;
			else           dm.Dradii[role-1][lines[role]][N-1] = r;

			//update line counter now that the full (= meaningful part of the) line is read
			++lines[role];

			//read the rest of the line (incl. the endline)
			f.getline(skipLine,512);

			//skip over lines with comments, as comments are allowed inside model blocks
			firstChar = f.get();
