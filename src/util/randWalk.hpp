...
class RandomWalk {
  public:
	...
	RandomWalk()
	    : rnd_ForceNewSeedsAng(-1), rnd_ForceNewSeedsZ(-1),
	      rnd_ForceNewSeedsDisp(-1) {
		...
		if (!rnd_StateAng || !rnd_StateZ || !rnd_StateDisp)
			throw report::rtError(
			    "Couldn't initialize random number generators.");
		...
		wlk_displacementBias_x = wlk_displacementBias_y =
		    wlk_displacementBias_z = 0.;
		wlk_xy_heading = 0.;
	...
	~RandomWalk() {
		...
		if (rnd_StateAng) {
			gsl_rng_free(rnd_StateAng);
			rnd_StateAng = NULL;
		}
		if (rnd_StateZ) {
			gsl_rng_free(rnd_StateZ);
			rnd_StateZ = NULL;
		}
		if (rnd_StateDisp) {
			gsl_rng_free(rnd_StateDisp);
			rnd_StateDisp = NULL;
		}
	}
	 ...
	 * of the walk turning angles (in radians), angles must be within
	 * [-3.14; 3.14], probability values must be non-negative.
	 *
	 * It can be either a histogram from a representant real walk or can be
	 * filled with theoretical distribution, see
	 * RandomWalk::UseWrappedCauchyDistribution().
	 *
	 * The values for unspecified angles will be interpolated from their
	 * "neighbors", it is assumed zero probability for angles less than -3.14 or
	 * more than +3.14.
	 *
	 * If an uniform distribution is provided, e.g. with
	 * RandomWalk::UseUniformDistribution(), the produced random walk will be
	 * isotropic (simulating Brownian motion), otherwise it will always be
	 * correlated to the previous move (simulating persistent walks).
	 *
	 * Note that a persistent time from walks simulated with this class can be
	 * calculated with this class only for the distributions generated with the
	 * function RandomWalk::UseWrappedCauchyDistribution(). For any other
	 * distribution, one must use some other SW (to fit theoretical
	 * persistence-driven curve to a measured MSD curve).
	 *
	 * This is the main input parameter of the walk. It must be, however,
	 * pre-processed with RandomWalk::PrepareRandomTurningField() for the walks
	 * to follow it.
	 *
	 * Note that it influences only the 2D portion (within the XY-plane) of a 3D
	 * walking. Note the (in principle) similar attribute
	 * RandomWalk::displacementStepSizeDistribution.
	 */
	...
	void UseUniformDistribution(void) {
		// relays on the interpolation in the
		// RandomWalk::PrepareRandomTurningField()
	...
	void UseCorrectedUniformDistribution(void) {
		turningAngleDistribution.clear();
		for (int i = 0; i < 180; ++i) {
			turningAngleDistribution[FT(i - 180) / 180. * 3.14] =
			    -18.0 * (float(i) / 180.) + 31.0;
			turningAngleDistribution[FT(i) / 180. * 3.14] =
			    18.0 * (float(i) / 180.) + 13.0;
		}
	}
	 ...
	 * isotropic random walks). The closer to 1 it is, the more straight the
	 * walks will be (and hence less random).
	 *
	 * Note that it influences only the 2D portion (within the XY-plane) of a 3D
	 * walking.
	 */
	void UseWrappedCauchyDistribution(const FT r) {
		...
		for (int i = 0; i <= 62; ++i) {
			...
			turningAngleDistribution[angle] =
			    0.159 * ((1. - r * r) / (1. + r * r - 2. * r * cos(angle)));
		}
	}
	 ...
	 * Sets the RandomWalk::turningAngleDistribution such that generated random
	 * walks will simulate correlated random walks. The underlying turning angle
	 * distribution will be due to the wrapped Gauss/normal (circular)
	 * distribution.
	 *
	 ...
	 * span of turning angles will be [-3*sigma,+3*sigma]. Very large sigma
	 * should produce something similar to isotropic random walk. For the mean
	 * cosine angle r it holds r=exp(-\e sigma*\e sigma/2). And,
	 * sigma=sqrt(-2*ln(r)).
	 *
	 * Note that it influences only the 2D portion (within the XY-plane) of a 3D
	 * walking.
  ...
  protected:
	/**
	 * This attribute is used for drawing random turning angles and is hence
	 * important!
	 *
	 * It is a helper array to provide turning angles: a uniform random number
	 * generator chooses index to this array and the value found there is the
	 * next turning angle. Clearly, the histogram of angle values present in
	 * this array should be the same as is given in the
	 * RandomWalk::turningAngleDistribution attribute. For convenience, use the
	 * RandomWalk::PrepareRandomTurningField() method to fill this attribute.
	 *
	 * The array should be such long that every "discrete angle" (assuming
	 * sampling rate of 1/6.28 radians (1 deg)) whose probability is non-zero
  ...
  public:
	 ...
	 *
	 * Note that it influences only the 2D portion (within the XY-plane) of a 3D
	 * walking. Note the (in principle) similar method
	 * RandomWalk::PrepareRandomStepSizeField().
	 */
	void PrepareRandomTurningField(void) {
		...
		//   stretch Factor such that the randomTurningField is not way too
		//   long, that is the Sum * Factor is circa 10000
		...
		typename std::map<FT, FT>::const_iterator it =
		    turningAngleDistribution.begin();
		while (it != turningAngleDistribution.end()) {
			if (it->first >= -3.14 && it->first <= 3.14) {
				distribution[(int)round((it->first + 3.14) * 360. / 6.28)] =
				    it->second;
#ifdef RNDWLK_DEBUG
		...
		while (lastRightInited < 361 && distribution[lastRightInited] == -1.)
			++lastRightInited;
		// note that distribution[360] != -1. (which renders the first condition
		// useless)
		...
		for (int i = 1; i < 360; ++i) {
			if (distribution[i] == -1.) {
				// found uninitialized angle, interpolate from lastLeft and
				// lastRight ones
				distribution[i] = FT(i - lastLeftInited) /
				                      FT(lastRightInited - lastLeftInited) *
				                      (distribution[lastRightInited] -
				...
				lastRightInited = i + 1;
				while (lastRightInited < 361 &&
				       distribution[lastRightInited] == -1.)
					++lastRightInited;
			}
	 ...
	 * the stepSize must not be negative, probability values must be
	 * non-negative too.
	 *
	 * This attribute can be typically filled from a histogram from a
	 * representant real walk, or can be filled with some theoretical
	 * distribution.
	 *
	 * The values for unspecified angles will be interpolated from their
	 * "neighbors", it is assumed zero probability for steps less than 0.0. The
	 * "right" limit (on maximum displacement size) must be explicitly given by
	 * setting it to zero probability: \e
	 * displacementStepSizeDistribution[maxStepSize]=0.f;
	 *
	 ...
	 * to follow it.
	 *
	 * Note the (in principle) similar attribute
	 * RandomWalk::turningAngleDistribution.
	 */
	...
	void UseNormalDistDisplacementStep(const FT mean, const FT sigma) {
		...
		for (int i = intStart; i <= intStop; ++i) {
			...
			displacementStepSizeDistribution[step] =
			    1. / (sigma * 2.506) *
  ...
  protected:
	/**
	 * This attribute is used for drawing random displacement step sizes and is
	 * hence important!
	 *
	 * It is a helper array to provide step sizes: a uniform random number
	 * generator chooses index to this array and the value found there is the
	 * next step size. Clearly, the histogram of step sizes values present in
	 * this array should be the same as is given in the
	 * RandomWalk::displacementStepSizeDistribution attribute. For convenience,
	 * use the RandomWalk::PrepareRandomStepSizeField() method to fill this
	 * attribute.
	 *
  ...
  public:
	 ...
	 * while this function considers given user stepping, the probabilities of
	 * step sizes missing in the distribution are "guessed" using linear
	 * interpolation. It is assumed that right edge value poses zero (0)
	 * probability.
	 *
	 * \param[in] stepSampling	user given granularity of step sizes
	 *
	 * The granularity should be fine such that the
	 * RandomWalk::displacementStepSizeDistribution will not tend to aggregate
	 * its sampling nodes due to overly large \e stepSampling -- this method is
	 * not accommodated for that.
	 *
	 * Note the (in principle) similar method
	 * RandomWalk::PrepareRandomTurningField().
	 */
	void PrepareRandomStepSizeField(const FT stepSampling = 1.) {
		// see the RandomWalk::PrepareRandomTurningField() for some more
		// comments...
		if (stepSampling <= 0.)
			throw report::rtError(
			    "The stepSampling parameter must be strictly positive.");
		...
		if (it->second != 0.) {
			displacementStepSizeDistribution[std::max(it->first, FT(0.)) +
			                                 stepSampling] = 0.;
			++it;
		}
		...
		int lastLeftInited = 0;
		int lastRightInited = 1;
		while (lastRightInited < distLength &&
		       distribution[lastRightInited] == -1.)
			++lastRightInited;
		// note that distribution[distLength-1] != -1. (which renders the first
		// condition useless)
		...
		for (int i = 1; i < distLength - 1; ++i) {
			if (distribution[i] == -1.) {
				// found uninitialized value, interpolate from lastLeft and
				// lastRight ones
				distribution[i] = FT(i - lastLeftInited) /
				                      FT(lastRightInited - lastLeftInited) *
				                      (distribution[lastRightInited] -
	...
	void SetBiasVector(const FT x, const FT y, const FT z = 0.) {
		wlk_displacementBias_x = x;
		wlk_displacementBias_y = y;
		wlk_displacementBias_z = z;
		report::debugMessage(fmt::format(
		    "new Bias vector: ({},{},{}) in microns", wlk_displacementBias_x,
		    wlk_displacementBias_y, wlk_displacementBias_z));
	}

	/// Get currently used bias 2D vector.
	void GetBiasVector(FT& x, FT& y) const {
		x = wlk_displacementBias_x;
		y = wlk_displacementBias_y;
	}
	...
	void GetBiasVector(FT& x, FT& y, FT& z) const {
		x = wlk_displacementBias_x;
		y = wlk_displacementBias_y;
		z = wlk_displacementBias_z;
	}

	/// Indicates whether simulated walks are biased.
	bool IsWalkBiased(void) const {
		return (wlk_displacementBias_x != 0. || wlk_displacementBias_y != 0. ||
		        wlk_displacementBias_z != 0.);
	}
	...
	void ResetXYHeading(FT newHeading) {
		...
		while (newHeading < 0.)
			newHeading += 6.28318;
		while (newHeading > 6.28318)
			newHeading -= 6.28318;

		wlk_xy_heading = newHeading;
		report::debugMessage(
		    fmt::format("new XY heading angle: {} radians", wlk_xy_heading));
	}

	/**
	 * Sets the 3D walk anisotropy (only in the direction of the z-axis).
	 * The value must be within 0 and 1 (inclusive) interval. Value of 0
	 * enforces no movement along the z-axis, and value of 1 allows for a
	 * movement, e.g., only along the z-axis (no xy-movement).
	 */
	void SetZstepLimit(FT zLimit) {
		...
		wlk_z_limit = zLimit;
		report::debugMessage(
		    fmt::format("new Z anisotropy value: {} [no units]", wlk_z_limit));
	}

	/// Returns the current 3D walk z-axis anisotropy.
	FT GetZstepLimit(void) const { return (wlk_z_limit); }
	...
	void DoOneStep2D(void) {
		...
		const FT deltaHeading = randomTurningField[(int)floor(
		    rnd_GetValueAng(0., randomTurningField.size() - 0.001))];

		// update the azimuth (absolute heading)
		wlk_xy_heading += deltaHeading;

		// choose random step size from the underlying distribution
		const FT dispSize = randomStepSizeField[(int)floor(
		    rnd_GetValueDisp(0., randomStepSizeField.size() - 0.001))];
	...
	/// Do one step of the 2D random walk and return fresh new generated
	/// displacement
	void DoOneStep2D(FT& x, FT& y) {
		DoOneStep2D();
		x = wlk_displacement_x;
		y = wlk_displacement_y;
	}
	...
	/// Do one step of the 3D random walk and return fresh new generated
	/// displacement
	void DoOneStep3D(FT& x, FT& y, FT& z) {
		DoOneStep3D();
		x = wlk_displacement_x;
		y = wlk_displacement_y;
		z = wlk_displacement_z;
	}

	/// Get the last 2D generated displacement; does no simulation (see
	/// RandomWalk::DoOneStep2D())
	void GetLastWalkedDisplacement(FT& dx, FT& dy) const {
		dx = wlk_displacement_x;
		dy = wlk_displacement_y;
	}

	/// Get the last 3D generated displacement; does no simulation (see
	/// RandomWalk::DoOneStep3D())
	void GetLastWalkedDisplacement(FT& dx, FT& dy, FT& dz) const {
		dx = wlk_displacement_x;
		dy = wlk_displacement_y;
		dz = wlk_displacement_z;
	}
  ...
  protected:
	...
	/// Current heading/azimuth (in radians, in the XY-plane) of this random
	/// walk.
	FT wlk_xy_heading;

	/// Current limit for the z-values in the 3D walk. This imposses anisotropy
	/// of the 3D walk. Value must be in [0,1].
	FT wlk_z_limit;
	...
	/// helper random number generator: Flat/uniform distribution within [\e
	/// A,\e B]
	...
	/// helper random number generator: Flat/uniform distribution within [
	/// -this->wlk_z_limit, this->wlk_z_limit ]
