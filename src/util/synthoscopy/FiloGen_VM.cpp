...
void PhaseII(i3d::Image3d<float>& fimg,
	...
	i3d::Vector3d<float> res_psf = psf.GetResolution().GetRes(),
	...
	report::debugMessage(
	    fmt::format("image size: {}", toString(fimg.GetSize())));
	report::debugMessage(
	    fmt::format("psf size: {}", toString(psfII.GetSize())));
	...
	for (int z = 0; z < (signed)blurred_texture.GetSizeZ(); ++z)
		for (int y = 0; y < (signed)blurred_texture.GetSizeY(); ++y)
			for (int x = 0; x < (signed)blurred_texture.GetSizeX();
			     ++x, ++f, ++p) {
				// background signal (inverted parabola)
				float distSq =
				    -(SQR((float)(x - xC)) + SQR((float)(y - yC))) / maxDist +
				    1.f;
				*f = *p * distSq;
			}
...
void PhaseIII(i3d::Image3d<float>& blurred,
	...
	report::debugMessage(
	    fmt::format("Image contains {} voxels", blurred.GetImageSize()));
	...
	for (size_t i = 0; i < blurred.GetImageSize(); i++, p++) {
		...
		*p += ENF *
		      ((float)GetRandomPoisson(noiseMean, rngPhotonNoise) - noiseMean);
		...
		*p += GetRandomGauss(0.0f, 1.0f, rngReadoutNoise);
		...
		// ADCoffset ... how many intensity levels are globally added to each
		// pixel
