...
void PrepareFinalPreviewImage(i3d::Image3d<PV>& phantom,
	...
	i3d::GaussIIR(phantom, 0.13f * phantom.GetResolution().GetX(),
	...
	for (; pF != pFlast; ++pF) {
		...
		const float noiseMean =
		    std::sqrt(*pF); // from statistics: shot noise = sqrt(signal)
	...
	report::message(
	    fmt::format("Resampling to resolution: {}", finalRes.GetRes()));

	i3d::ResampleToDesiredResolution(phantom, finalRes, i3d::NEAREST_NEIGHBOUR);
	// i3d::ResampleToDesiredResolution(mask,    finalRes,
	// i3d::NEAREST_NEIGHBOUR);
