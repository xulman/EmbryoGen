...
// the cpp file, no one outside can request these to exist (and the
// compiler+linker will happily use the generic code instead of these
// specializations)
template Vector3d<int>&
Vector3d<int>::toPixels(const Vector3d<float>& floatPxPos);
template Vector3d<int>&
Vector3d<int>::toPixels(const Vector3d<double>& floatPxPos);
template Vector3d<size_t>&
Vector3d<size_t>::toPixels(const Vector3d<float>& floatPxPos);
template Vector3d<size_t>&
Vector3d<size_t>::toPixels(const Vector3d<double>& floatPxPos);

/** template specialization for Vector3d<size_t>::toPixels(), this
   specialization is avoiding std::floor() because there's nothing to round
   (we're integer type) */
template <>
