...
struct LabelSet
{
	...
	LabelSet(const EL eLabel, const OL a)
		: label(eLabel), array{{a}} {}

	//construct from two original labels
	LabelSet(const EL eLabel, const OL a,const OL b)
		: label(eLabel), array()
	{
		array.reserve(2);
		...
		{
			array.push_back(a);
			if (a != b) array.push_back(b);
		}
		else
		{
			array.push_back(b);
			if (a != b) array.push_back(a);
		}
	}
	...
	std::vector<OL> array;
	//TODO make it read-only -- either as reference to be created from read-write array, or make private and provide public inline operator[] const()
...
public:
	...
	EL add(const EL foundEL, const OL insertingOL)
	{
		...
		if (foundEL == 0) return (EL)insertingOL;

		//adding to voxel that contained just one original label?
		if (foundEL < firstEncodingLabel)
		{
			//adding over myself...
			if (foundEL == (EL)insertingOL) return foundEL;

			//construct the desired (and possibly new) multilabel
			LabelSet<EL,OL> possiblyNewEL(lastUsedEncodingLabel+1, foundEL,insertingOL);
		...
		//construct the desired (and possibly new) multilabel
		LabelSet<EL,OL> possiblyNewEL(lastUsedEncodingLabel+1, *existingEL,insertingOL);
		return add(possiblyNewEL);
	}
	...
	EL addAndUpdateUsage(const EL foundEL, const OL insertingOL)
	{
		...
		//TODO: notify all owners of the original labels that were in *existingEL
		//      that they are now represented with encoding label that is going to
	...
	const LabelSet<EL,OL>* getLabelSet(const std::vector<OL>& someOLs) const
	{
		...
		{
			if (ml.second.array.size() != someOLs.size()) continue;

			std::size_t i=0;
			for (; i < someOLs.size(); ++i)
				if (ml.second.array[i] != someOLs[i]) break;

			if (i == someOLs.size()) return &(ml.second);
		}
	...
	void printStatus(void) const
	{
		...
		EL usedELs = lastUsedEncodingLabel >= firstEncodingLabel ?
		                       lastUsedEncodingLabel-firstEncodingLabel+1 : 0;
...
private:
	...
	EL add(LabelSet<EL,OL>& possiblyNewEL)
	{
		...
		if (ml == NULL)
		{
			...
			++lastUsedEncodingLabel; //NB: now matches/equals possiblyNewEL.label
