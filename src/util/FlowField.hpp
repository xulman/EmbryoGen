...
struct FlowField {
  public:
	...
	FlowField(FlowField<FT> const& FF) {
		if (FF.x)
			x = new i3d::Image3d<FT>(*FF.x);
		else
			x = NULL;
		if (FF.y)
			y = new i3d::Image3d<FT>(*FF.y);
		else
			y = NULL;
		if (FF.z)
			z = new i3d::Image3d<FT>(*FF.z);
		else
			z = NULL;
	}
	...
	void
	proxify(i3d::Image3d<FT>* _x, i3d::Image3d<FT>* _y, i3d::Image3d<FT>* _z) {
		x = _x;
		y = _y;
		z = _z;
		canDeleteXYZ = false;
	...
	bool isConsistent(void) const {
		...
		if (x->GetSize() != y->GetSize())
			return false;
		if (x->GetResolution().GetRes() != y->GetResolution().GetRes())
			return false;
		...
		if (z) {
			if (x->GetSize() != z->GetSize())
				return false;
			if (x->GetResolution().GetRes() != z->GetResolution().GetRes())
				return false;
			if (x->GetOffset() != z->GetOffset())
				return false;
		}
		return true;
	}
	 ...
	 * offsets, which is a vector, must be, axes-wise, an integer multiple of
	 * pixels. The later test for "being an integer vector" is performed by
	 * element-wise subtracting roundf() of respective vector element and
	 * testing whether the squared size of the residual vector is smaller than
	 * \e precision2, which is again a squared value.
	 *
	...
	bool isAligned(FlowField<FT> const& FF, const FT precision2 = (FT)0.0001) {
		...
		offDiff.x *= res.x;
		offDiff.x -= roundf(offDiff.x);
		offDiff.y *= res.y;
		offDiff.y -= roundf(offDiff.y);
		offDiff.z *= res.z;
		offDiff.z -= roundf(offDiff.z);
		...
		report::debugMessage(
		    fmt::format("Sq. difference of grid offsets is {} in pixels, "
		                "allowed sq. precision is {} in pixels",
		                i3d::Norm2(offDiff), precision2));
	...
	int drawFF(DisplayUnit& du,
		...
		for (pxPos.z = 0; pxPos.z < x->GetSizeZ(); pxPos.z += sparsity.z)
			for (pxPos.y = 0; pxPos.y < x->GetSizeY(); pxPos.y += sparsity.y)
				for (pxPos.x = 0; pxPos.x < x->GetSizeX();
				     pxPos.x += sparsity.x) {
