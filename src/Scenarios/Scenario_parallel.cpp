...
float getDiagonalAgentsAABBdistance(const float dx,
                                    const float dy,
                                    const float r) {
	// corners distance
	float rdx = dx - 2 * r;
...
class ParallelNucleus : public AbstractAgent {
  public:
...
#ifndef DEBUG
		report::message(
		    fmt::format("{} created: \"{}\"", getSignature(), _type));
#endif
	}
	...
	/// flag to tell publishGeometry() to update the geometry (to prevent it
	/// from translating twice)
	...
	std::vector<const ShadowAgent*> nearbyAgents;

	/// internal affairs: flag that the agent should move
	void advanceAndBuildIntForces(const float) {
		// random duration (in full 2-10 seconds) pause here to pretend "some
		// work"
		const int waitingTime = (int)GetRandomUniform(0, 20);
		report::message(
		    fmt::format("{} pretends work that would last for {} milisecond(s)",
		                getSignature(), waitingTime));
		std::this_thread::sleep_for(
		    std::chrono::milliseconds((long long)waitingTime));
	...
	// we don't utilize any extra (2nd) geometry to be updated, and we cannot
	// update the official this.geometry here (that can happen only in
	// publishGeometry()), so these methods are empty
	...
	void publishGeometry(void) {
		...
		if (shouldUpdateGeometry && x % 18 == 17 && y % 18 == 17 &&
		    currTime > 0.2 && currTime < 1.9) {
			report::debugMessage(
			    fmt::format("\"{}\" failed to provide updated geometry",
			                this->agentType.getString()));
	...
	void drawTexture(i3d::Image3d<float>& phantom,
	                 i3d::Image3d<float>& indices) {
		// report the current time into the phantom
		phantom.SetVoxel((size_t)x, (size_t)y, 0, currTime);

		// if this call is the very first snapshot (the one after the scene is
		// build, and no simulation round has been executed so far), the
		// neighbors have not yet examined
		if (currTime == 0)
			collectExtForces();

		// report the encoded list of IDs of nearby agents with the purpose to
		// spot if the constellation around this agent has changed... (as a
		// result of badly updated info about surrounding bboxes)
		//
		// how to encode n IDs into scalar? f(a1,...,an) =
		//  = sum_i ai -> no, different agents can easily give the same
		//  encoding, e.g. 19+20 = 18+21 = prod_i ai -> no, with large grids
		//  (with large ID numbers) the product may not fit into float = sum_i
		//  hash(ai) -> yes, provided n times hashDomainSizeInBits fits into
		//  float
		//                           (hash(19) != hash(20) != hash(18) !=
		//                           hash(21) and
		//                            also very unlikely hash(19)+hash(20) ==
		//                            hash(18)+hash(21))
		size_t hash = 0;
		for (const auto sa : nearbyAgents) {
			...
			if (nearbyAgents.size() > 4)
				report::message(fmt::format("{} sees around agent ID {}",
				                            agentType.getString(), ID));
...
std::unique_ptr<SceneControls> Scenario_Parallel::provideSceneControls() const {
	...
	class mySceneControl : public SceneControls {
	  public:
		...
		void updateControls(const float currTime) override {
			if (currTime == 0.1f) {
				report::debugMessage(
				    "stopping the production of the maskXXX.tif files");
	...
	// ctrl->displayUnit.RegisterUnit( new
	// FlightRecorderDisplayUnit("FlightRecording.txt") );
	// ctrl->displayUnit.RegisterUnit( new
	// SceneryBufferedDisplayUnit("localhost:8765") );
	// ctrl->displayUnit.RegisterUnit( new
	// SceneryBufferedDisplayUnit("10.1.202.7:8765") );     //laptop @ Vlado's
	// office ctrl->displayUnit.RegisterUnit( new
...
void Scenario_Parallel::initializeAgents(FrontOfficer* fo, int p, int P) {
	...
	for (int y = 0; y < howManyAlongY; ++y)
		for (int x = 0; x < howManyAlongX; ++x) {
			...
			//report::message(fmt::format("Parallel Scenario created
			agents/batch size = {} p={} howManyAlongX={} howManyAlongY={}",
			createdAgents/batchSize+1, p, howManyAlongX, howManyAlongY)); if
			((createdAgents-1)/batchSize +1 < p) continue;
