...
// definitions loop, and the same holds for the Director type
class FrontOfficer;
class Director;
class Scenario;
template <typename T>
void transferImgs(
    const i3d::Image3d<T>& img /*,  DAIS::ImagesAsEventsSender& channel */);
...
class SceneControls {
  ...
  public:
	...
	/** c'tor that essentially allows to set the const'ed attributes with
	...
	virtual void updateControls(const float) {
		report::debugMessage("This scenario is not updating its controls.",
		                     {false});
	}
	...
	void displayChannel_createNew(
		...
		//	= new
		// DAIS::ImagesAsEventsSender(destinationURL,30,channelDisplayTitle);
	}
	...
	void displayChannel_enableForImgMask(const std::string& channelName) {
		imgMaskBroadcast.insert(channelName);
	}
	void displayChannel_disableForImgMask(const std::string& channelName) {
		imgMaskBroadcast.erase(channelName);
	}
	void displayChannel_transferImgMask() {
		transferImg(imgMask, "mask", imgMaskBroadcast);
	}

	void displayChannel_enableForImgPhantom(const std::string& channelName) {
		imgPhantomBroadcast.insert(channelName);
	}
	void displayChannel_disableForImgPhantom(const std::string& channelName) {
		imgPhantomBroadcast.erase(channelName);
	}
	void displayChannel_transferImgPhantom() {
		transferImg(imgPhantom, "phantom", imgPhantomBroadcast);
	}

	void displayChannel_enableForImgOptics(const std::string& channelName) {
		imgOpticsBroadcast.insert(channelName);
	}
	void displayChannel_disableForImgOptics(const std::string& channelName) {
		imgOpticsBroadcast.erase(channelName);
	}
	void displayChannel_transferImgOptics() {
		transferImg(imgOptics, "optics", imgOpticsBroadcast);
	}

	void displayChannel_enableForImgFinal(const std::string& channelName) {
		imgFinalBroadcast.insert(channelName);
	}
	void displayChannel_disableForImgFinal(const std::string& channelName) {
		imgFinalBroadcast.erase(channelName);
	}
	void displayChannel_transferImgFinal() {
		transferImg(imgFinal, "final", imgFinalBroadcast);
	}
	...
	void imagesSaving_disableForImgOptics() {
		disableProducingOutput(imgOptics);
	};
  ...
  protected:
	...
	template <typename T>
	void transferImg(const i3d::Image3d<T>& /* img */,
	                 const char* imgName,
		...
		while (route != routes.end()) {
			...
			if (channel != transferChannels.end()) {
				...
				//      << channel->first << "\" (" << channel->second->getURL()
				//      << "), hold on...");
				...
				report::debugMessage(fmt::format(
				    "Deleting incorrect \"{}\" from channels for {} image",
				    *route, imgName));
  ...
  public:
	...
	setOutputImgSpecs(const Vector3d<float>& imgOffsetInMicrons,
		...
		// metadata...
		imgMask.SetResolution(
		    i3d::Resolution(imgResolutionInPixelsPerMicron.toI3dVector3d()));
		...
		lastUsedImgSize.from(Vector3d<float>(imgSizeInMicrons)
		                         .elemMult(imgResolutionInPixelsPerMicron)
		                         .elemCeil());
  ...
  protected:
	...
	void enableProducingOutput(i3d::Image3d<T>& img) {
		if ((long)&img == (long)&imgFinal && !isProducingOutput(imgPhantom))
			report::message(fmt::format("WARNING: Requested synthoscopy but "
			                            "phantoms may not be produced."));

		report::debugMessage(
		    fmt::format("allocating {} MB of memory for image of size {} px",
		                (lastUsedImgSize.x * lastUsedImgSize.y *
		                 lastUsedImgSize.z / (1 << 20)) *
		                    sizeof(*img.GetFirstVoxelAddr()),
		                toString(lastUsedImgSize)));
	...
	void disableProducingOutput(i3d::Image3d<T>& img) {
		// image size flags if this image should be iteratively filled and
		// saved: zero image size signals "don't use this image"
		img.MakeRoom(0, 0, 0);
	}
  ...
  public:
	...
	/** reference back on the containing scenario that this object is
	 * controlling */
	...
	SceneControls& setCtx(Scenario* ctx) {
		scenario = ctx;
		return *this;
	}
};
...
class Scenario {
  ...
  public:
	struct Displays {
		...
		void registerImagingUnit(
		    const std::string& unitNickName,
		    const char* destinationURL,
		    const char* thisSourceNickName = "EmbryoGen's Image(s)") {
			if (ctx.amIinDirektorContext())
				params.displayChannel_createNew(unitNickName, destinationURL,
				                                thisSourceNickName);
		}
	...
	struct Disks {
		...
		void enableImgMaskTIFFs() { params.imagesSaving_enableForImgMask(); }
		void disableImgMaskTIFFs() { params.imagesSaving_disableForImgMask(); }
		...
		void enableImgFinalTIFFs() { params.imagesSaving_enableForImgFinal(); }
