...
class myNucleusC : public NucleusAgent, CellCycle {
  public:
	myNucleusC(const int _ID,
	           const std::string& _type,
	           const Spheres& shape,
	           const float _currTime,
	           const float _incrTime)
	    : NucleusAgent(_ID, _type, shape, _currTime, _incrTime), CellCycle(8) {
		// debug report
		startCycling(_currTime);
		if (ID == 1)
			reportPhaseDurations();
	}
	...
	void runG1Phase(const float time) override {
		report::debugMessage(fmt::format("time={}", time));
	}
	...
	void runSPhase(const float time) override {
		report::debugMessage(fmt::format("time={}", time));
	}
	...
	void runG2Phase(const float time) override {
		report::debugMessage(fmt::format("time={}", time));
	}
	...
	void runProphase(const float time) override {
		report::debugMessage(fmt::format("time={}", time));
	}
	...
	void runMetaphase(const float time) override {
		report::debugMessage(fmt::format("time={}", time));
	}
	...
	void runTelophase(const float time) override {
		report::debugMessage(fmt::format("time={}", time));
	}
	void closeTelophase(void) override { report::debugMessage("do some work"); }

	void startCytokinesis(void) override {
		report::debugMessage("do some work");
	}
	void runCytokinesis(const float time) override {
		report::debugMessage(fmt::format("time={}", time));
	}
