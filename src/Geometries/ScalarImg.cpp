...
void ScalarImg::getDistance(
	...
	case ListOfShapeForms::Mesh:
		// TODO: attempt to project mesh vertices into the mask image and look
		// for collision
		report::message(
		    fmt::format("this.ScalarImg vs Mesh is not implemented yet!"));
		break;
	case ListOfShapeForms::ScalarImg:
		// TODO identity case
		report::message(
		    fmt::format("this.ScalarImg vs ScalarImg is not implemented yet!"));
		// getDistanceToScalarImg((ScalarImg*)&otherGeometry,l);
		break;
	case ListOfShapeForms::VectorImg:
		report::message(
		    fmt::format("this.ScalarImg vs VectorImg is not implemented yet!"));
			...
			for (curPos.x = minSweepPX.x; curPos.x < maxSweepPX.x; curPos.x++) {
				...
				for (std::size_t i = 0; i < io; ++i) {
					...
					if (std::abs(surfPoint.len() - radiiO[i]) < lenPXhd) {
						...
						// a vector pointing from voxel's centre to the nearest
						// sphere surface
						surfPoint *= radiiO[i] / surfPoint.len() - 1.0f;
						// NB: surfPoint *= (radiiO[i] - surfPoint.len()) /
						// surfPoint.len()

						// max (squared; only to avoid using std::abs())
						// distance to the voxel's centre to see if surfPoint's
						// tip is within this voxel
						surfPoint.elemMult(surfPoint);
						if (surfPoint.x <= vecPXhd2.x &&
						    surfPoint.y <= vecPXhd2.y &&
						    surfPoint.z <= vecPXhd2.z) {
	...
	for (std::size_t i = 0; i < io; ++i)
		if (distances[i] < TOOFAR) {
			...
			if (hints[i].x + 1 < distImg.GetSizeX())
				grad.x =
				    distImg.GetVoxel(hints[i].x + 1, hints[i].y, hints[i].z);
			else
			...
			// NB: this stretches the value difference as if central difference
			// is calculated,
			//     and leaves it at zero when GetSizeX() == 1 because grad.x ==
			//     defValue - defValue

			span = 2;
			if (hints[i].y + 1 < distImg.GetSizeY())
				grad.y =
				    distImg.GetVoxel(hints[i].x, hints[i].y + 1, hints[i].z);
			else
			...
			if (hints[i].z + 1 < distImg.GetSizeZ())
				grad.z =
				    distImg.GetVoxel(hints[i].x, hints[i].y, hints[i].z + 1);
			else
			...
			// DEBUG_REPORT("grad: " << grad << " @ voxel: " << hints[i] << ",
			...
			// determine the exact point on the sphere surface, use again the
			// voxel's centre
			surfPoint.toMicronsFrom(
			    hints[i], distImgRes,
			    distImgOff); // now real world coordinate of the pixel's centre
			...
			// this is from ScalarImg perspective (local = ScalarImg, other =
			// Sphere), it reports index of the relevant foreign sphere
			l.emplace_back(
			    surfPoint + grad, surfPoint, distances[i],
			    (signed)distImg.GetIndex(hints[i].x, hints[i].y, hints[i].z),
			    i);
		}
...
void ScalarImg::deserialize(std::span<const std::byte> bytes) {
	...
	if (mmodel != (int)this->model)
		throw report::rtError(
		    fmt::format("Deserialization mismatch: filling image of dist. "
		                "model {} with model {} from the buffer",
		                this->model, (DistanceModel)mmodel));
...
template void
ScalarImg::updateWithNewMask(const i3d::Image3d<i3d::GRAY8>& _mask);

template void
ScalarImg::updateWithNewMask(const i3d::Image3d<i3d::GRAY16>& _mask);

template void ScalarImg::updateWithNewMask(const i3d::Image3d<float>& _mask);

template void ScalarImg::updateWithNewMask(const i3d::Image3d<double>& _mask);

void ScalarImg::renderIntoMask(i3d::Image3d<i3d::GRAY16>&,
                               const i3d::GRAY16) const {
