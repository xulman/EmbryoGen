                                     ...
                                     std::list<ProximityPair>& l) const
{
	...
	Vector3d<G_FLOAT> vecPXhd2(0.5f/distImgRes.x,0.5f/distImgRes.y,0.5f/distImgRes.z);
		...
		for (int i = 0; i < io; ++i)
		{
			...
			if (std::abs(surfPoint.len() - radiiO[i]) < lenPXhd)
			{
				...
				if (surfPoint.x <= vecPXhd2.x && surfPoint.y <= vecPXhd2.y && surfPoint.z <= vecPXhd2.z)
				{
					...
					const G_FLOAT dist = distImg.GetVoxel(curPos.x,curPos.y,curPos.z);
	...
	if (distances[i] < TOOFAR)
	{
		...
		const G_FLOAT defValue = distImg.GetVoxel(hints[i].x,hints[i].y,hints[i].z);
...
void ScalarImg::updateThisAABB(AxisAlignedBoundingBox& AABB) const
{
	if (model == GradIN_ZeroOUT)
	{
		...
		const Vector3d<G_FLOAT> oneVxSize( Vector3d<G_FLOAT>(1).elemDivBy(distImgRes) );
