...
class InnerAxes {
  public:
	InnerAxes(void) : mA(1, 0, 0), sA(0, 1, 0), tA(0, 0, 1) {}
	...
	/** establish the main, secondary and ternary axes from the given one axis:
	     the main axis becomes the skeleton, the secondary axis is derived from
	   the skeleton, and the ternary is a vector product of the two; the
	   secondary is the main axis in which min-valued element is replaced with
	   the value of the max element, mid-valued element is preserved, and
	   max-valued element is calculated to satisfy perpendicularity */
	void setFromSkeletonAxis(const Vector3d<FT>& skeleton) {
		// skip if zero or essentially-zero skeleton vector is provided
		if (skeleton.len2() <= 0.0001)
			return;
		...
		if (va != 0)
			tmp[ia] = -(skeleton.x * tmp[0] + skeleton.y * tmp[1] +
			            skeleton.z * tmp[2]) /
			          va;
	...
	bool isOrthogonalBase(void) {
		if (std::abs(dotProduct(mA, sA)) > 0.0001)
			return false;
		if (std::abs(dotProduct(mA, tA)) > 0.0001)
			return false;
		if (std::abs(dotProduct(sA, tA)) > 0.0001)
			return false;
		return true;
	}
	...
	bool isOrthonormalBase(void) {
		if (!isOrthogonalBase())
			return false;
		if (mA.len2() > 0.0000001)
			return false;
		if (sA.len2() > 0.0000001)
			return false;
		if (tA.len2() > 0.0000001)
			return false;
		return true;
	}
	...
	void rotateAxes(const Vector3d<FT>& oldDirection,
		...
		// don't do anything if there's nearly no difference between the old and
		// new directions
		...
		Vector3d<FT> rotAxis =
		    crossProduct(oldDirection, newDirection).changeToUnitOrZero();
	...
	inline void rotateVector(Vector3d<FT>& v) {
	...
	FT rotMatrix[9];
	//
	void setupRotMatrixFromQuaternion(const FT q0,
	                                  const FT q1,
	                                  const FT q2,
	                                  const FT q3) {
		// from this URL:
