...
/**
 * The class essentially represents a 3D vector field that spans a certain
 * volume.
 *
 * The class was designed with the assumption that agents who interact with this
 * agent will want to learn about vectors at their surface. So, during
 * construction or later, the caller can decide what policy to apply during
 * Geometry::getDistance() calls. The policies to choose from can be, e.g.,
 * return min/max, average or all vectors that exists at the surface of the
 * other agent. The vectors are reported in a form of ProximityPairs. If other
 * policy is demanded, one does not need to call getDistance() at all (or
...
class VectorImg : public Geometry {
  ...
  private:
	VectorImg(ChoosingPolicy _policy)
	    : Geometry(ListOfShapeForms::VectorImg),
	      policy(_policy) { /** createAndDeserializeFrom() will do it for us */
	}
  ...
  public:
	 ...
	 */
	void saveFFx(const char* fullFilename) { X.SaveImage(fullFilename); }

	/** just for debug purposes: save the vector field.y image to a fullFilename
	 */
	void saveFFy(const char* fullFilename) { Y.SaveImage(fullFilename); }

	/** just for debug purposes: save the vector field.z image to a fullFilename
	 */
	void saveFFz(const char* fullFilename) { Z.SaveImage(fullFilename); }
	...
	void saveFF(const char* filenamesPrefix) {
		char fn[1024];
		sprintf(fn, "%s_x.ics", filenamesPrefix);
		X.SaveImage(fn);
		sprintf(fn, "%s_y.ics", filenamesPrefix);
		Y.SaveImage(fn);
		sprintf(fn, "%s_z.ics", filenamesPrefix);
		Z.SaveImage(fn);
	}

	/** just for debug purposes: save the image with magnitudes of the vectors
	 * to a filename */
	...
	/** Specialized implementation of getDistance() for VectorImg & Spheres
	   ...
	   and, based on this->policy finds the right distance for every other
	   sphere. These surface distances and corresponding ProximityPairs are
	   added to the output list l.

	    Every ProximityPair returned here is in fact encoding some vector from
	   the VectorImg, it is no real distance to anything. The 'other' position
	   in the ProximityPair is where the other surface is hitting this
	   VectorImg, and displacement from 'other' to 'local' position in the
	   ProximityPair is the vector itself. It is assumed that the other
	   interacting party knows well how to interpret it.

	    One could say that the ProximityPairs are hijacked in this special type
	   of "geometry", and indeed it is true. The reason for it is that the only
	   interaction between agents is allowed (currently) via
	   ShadowAgent::geometry attribute, which supports communication by means of
	   ProximityPairs (via Geometry::getDistance()). Nevertheless, one is always
  ...
  public:
	...
	static VectorImg createAndDeserialize(std::span<const std::byte> bytes);
