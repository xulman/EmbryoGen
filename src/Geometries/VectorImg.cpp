...
void VectorImg::getDistance(const Geometry& otherGeometry,
	...
	case ListOfShapeForms::Mesh:
		...
		// for collision
		report::message(
		    fmt::format("this.VectorImg vs Mesh is not implemented yet!"));
		break;
	...
	case ListOfShapeForms::VectorImg:
		// TODO identity case
		report::message(
		    fmt::format("this.VectorImg vs VectorImg is not implemented yet!"));
				...
				for (int i = 0; i < io; ++i) {
					...
					if (std::abs(surfPoint.len() - radiiO[i]) < lenPXhd) {
						...
						if (surfPoint.x <= vecPXhd2.x &&
							...
							// i-th sphere's surface let's inspect the FF's
							...
							case allVec:
								...
								l.emplace_back(
								    surfPoint + vec, surfPoint, vec.len(),
								    (signed)X.GetIndex(curPos.x, curPos.y,
								                       curPos.z),
								    i);
								break;
							default:;
	...
	if (policy == minVec) {
		...
		for (int i = 0; i < io; ++i)
			if (lengths2[i] < TOOFAR) {
				// found one
				// calculate exact point on the sphere's surface (using
				// hints[i])
				...
				// this is from VectorImg perspective (local = VectorImg, other
				// = Sphere), it reports index of the relevant foreign sphere
				l.emplace_back(
				    surfPoint + vecs[i], surfPoint, std::sqrt(lengths2[i]),
		...
		for (int i = 0; i < io; ++i)
			if (lengths2[i] > 0) {
				...
				// this is from VectorImg perspective (local = VectorImg, other
				// = Sphere), it reports index of the relevant foreign sphere
				l.emplace_back(centresO[i] + vecs[i], centresO[i],
				               vecs[i].len(), 0, i);
				// NB: average vector is no particular existing vector, hence
				// local hint = 0,
...
void VectorImg::deserializeFrom(char* buffer) {
	...
	if (ppolicy != (int)this->policy)
		throw report::rtError(
		    fmt::format("Deserialization mismatch: filling image of choosing "
		                "policy model {} with model {} from the buffer",
		                this->policy, (ChoosingPolicy)ppolicy));
...
void VectorImg::renderIntoMask(i3d::Image3d<i3d::GRAY16>&,
