  ...
  public:
	FlightRecorderDisplayUnit(const char* filename) {
		...
		if (testingFile.is_open()) {
			testingFile.close();
			throw report::rtError("Refusing to create 101st flight record, "
			                      "please clean up your disk.");
		}
	...
	~FlightRecorderDisplayUnit() { logger.close(); }
	...
	void DrawPoint(const int ID,
		...
		logger << "v1 points 1 dim 3 " << ID << " " << pos.x << " " << pos.y
		       << " " << pos.z << " " << radius << " " << color << "\n";
	}
	...
	void DrawLine(const int ID,
		...
		// where o1,...,oD,p1,...,pD are real scalars, ID,color are integer
		// scalars
		//
		// o,p are two end points of a line

		logger << "v1 lines 1 dim 3 " << ID << " " << posA.x << " " << posA.y
		       << " " << posA.z << " " << posB.x << " " << posB.y << " "
		       << posB.z << " " << color << "\n";
	}
	...
	void DrawVector(const int ID,
		...
		// scalars
		//
		// o is the base coordinate/anchor point of the vector, p is the vector
		// itself
		//
		// vector  ID   essentially determines to which cell it belongs
		// vector color essentially determines what type this vector is

		logger << "v1 vectors 1 dim 3 " << ID << " " << pos.x << " " << pos.y
		       << " " << pos.z << " " << vector.x << " " << vector.y << " "
		       << vector.z << " " << color << "\n";
	}
	...
	void DrawTriangle(const int ID,
	                  ...
	                  const int color = 0) override {
		//     message syntax := v1 triangles N dim D PointTripple1 ...
		//     PointTrippleN
		...
		logger << "v1 triangles 1 dim 3 " << ID << " " << posA.x << " "
		       << posA.y << " " << posA.z << " " << posB.x << " " << posB.y
		       << " " << posB.z << " " << posC.x << " " << posC.y << " "
		       << posC.z << " " << color << "\n";
	}
	...
	void Flush(void) override { logger.flush(); }
