...
void SceneryDisplayUnit::DrawPoint(const int ID,
	...
	msg << "v1 points 1 dim 3 " << ID << " " << pos.x << " " << pos.y << " "
	    << pos.z << " " << radius << " " << color;
...
void SceneryDisplayUnit::DrawLine(const int ID,
	...
	msg << "v1 lines 1 dim 3 " << ID << " " << posA.x << " " << posA.y << " "
...
void SceneryDisplayUnit::DrawVector(const int ID,
	...
	msg << "v1 vectors 1 dim 3 " << ID << " " << pos.x << " " << pos.y << " "
	    << pos.z << " " << vector.x << " " << vector.y << " " << vector.z << " "
	    << color;
...
void SceneryDisplayUnit::DrawTriangle(const int ID,
	...
	msg << "v1 triangles 1 dim 3 " << ID << " " << posA.x << " " << posA.y
	    << " " << posA.z << " " << posB.x << " " << posB.y << " " << posB.z
	    << " " << posC.x << " " << posC.y << " " << posC.z << " " << color;
...
report::debugMessage(fmt::format("Connecting Scenery DU to: {}" ,
(std::string("tcp://")+std::string(hostUrl))));
...
void SceneryDisplayUnit::DisconnectFromHost(void) {
	...
	if (socket != NULL)
	{
report::debugMessage(fmt::format("Disconnecting Scenery DU from: {}" ,
(std::string("tcp://")+std::string(hostUrl))));
