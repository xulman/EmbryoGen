...
class DisplayUnit {
  public:
	...
	virtual void DrawPoint(const int ID,
	...
	/** Draws a line between two points 'posA' and 'posB'.
	    ...
	    with its fixed ID, so that the displaying side can update it. */
	virtual void DrawLine(const int ID,
	...
	/** Draws a 'vector' often as an arrow, positioned at 'pos'.
	    ...
	    with its fixed ID, so that the displaying side can update it. */
	virtual void DrawVector(const int ID,
	                        const Vector3d<float>& pos,
	...
	/** Draws a triangle between the three points. The order of them
	    ...
	    with its fixed ID, so that the displaying side can update it. */
	virtual void DrawTriangle(const int ID,
	                          const Vector3d<float>& posA,
	                          const Vector3d<float>& posB,
	...
	/** A free format message to be sent over to the display unit. The
	   idea/assumption is that this message is sent typically after one
	...
	/** See docs of AbstractAgent::drawMask() */
	static inline int firstIdForAgentDebugObjects(const int agentID) {
		return agentID << 17 | 1 << 16; /* enable debug bit */
	}

	/** See docs of AbstractAgent::drawMask() */
	static inline int firstIdForSceneDebugObjects() { return 0; }
};
